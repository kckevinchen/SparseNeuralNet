...
class Resnet(tf.keras.Model):
  def __init__(self, block, num_blocks, num_classes=10, temp=1.0, in_planes=64, stable_resnet=False , sparse_level = 3, save_input_shape=False):
    ...
    self.conv1 = GeneralConv2D(self.in_planes, kernel_size=3, stride=1, padding=1, bias=False,sparse_level = self.sparse_level, save_input_shape=save_input_shape)
  ...
  def _make_layer(self, block, planes, num_blocks, stride):
    ...
    for stride in strides:
      layers.append(block(self.in_planes, planes, stride, self.L,sparse_level = self.sparse_level, save_input_shape=self.save_input_shape))
